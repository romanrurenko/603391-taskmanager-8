...
const handler = () => {
  ...
  clearBoards();
  showBoards(getRandom(1, 8));

};

const getBoardElement = (type, caption) =>
  `<article class="card card--pink card--${type}">
...
const showBoards = (count) => {
  for (let i = 0; i < count; i++) {
    boardsContainer.insertAdjacentHTML(`beforeend`, getBoardElement(boardsData[0].type, boardsData[0].caption));
  }
};

for (const it of filtersData) {
  const filtersContainer1 = document.querySelector(`.main__filter`);
  filtersContainer1.insertAdjacentHTML(`beforeend`, getFilterElement(it.caption, it.amount, it.checked));
